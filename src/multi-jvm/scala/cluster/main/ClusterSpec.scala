...
object ClusterSpecConfig extends MultiNodeConfig {
  ...
  val roleFrontend = role(name="roleFrontend")
  ...
  def roleList = Seq(roleFrontend, roleManage, roleCompute1, roleCompute2, roleCompute3)
  ...
  nodeConfig(roleFrontend)(buildConfig(role = "frontend", port = 2551))
  nodeConfig(roleManage)(buildConfig(role = "manage",     port = 2552))
  nodeConfig(roleCompute1)(buildConfig(role = "compute",  port = 2553))
  nodeConfig(roleCompute2)(buildConfig(role = "compute",  port = 2554))
  nodeConfig(roleCompute3)(buildConfig(role = "compute",  port = 2555))
}
...
abstract class ClusterSpec extends MultiNodeSpec(ClusterSpecConfig)
  ...
  "The cluster" must {
    "run a test dataset" in within(max = 5 minutes) {
      ...
      runOn(roleManage) {
        system.actorOf(ManageActor.props(new DatabaseDao()), name = "manage")
      }

      runOn(roleCompute1, roleCompute2, roleCompute3) {
        system.actorOf(ComputeActor.props(parallelization = 1, dao = new DatabaseDao()), name = "compute")
      }
