...
object MergeHub {
  ...
  /**
    ...
    */
  def source[T](perProducerBufferSize: Int, completeWhenAllUpstreamsComplete: Boolean = false): Source[T, Sink[T, NotUsed]] =
    Source.fromGraph(new MergeHub[T](perProducerBufferSize, completeWhenAllUpstreamsComplete))
...
/**
  * INTERNAL API
  */
private[streams] class MergeHub[T](perProducerBufferSize: Int, completeWhenAllUpstreamsComplete: Boolean) extends GraphStageWithMaterializedValue[SourceShape[T], Sink[T, NotUsed]] {
  ...
  private final case class CompletedStage(id: Long) extends Event
  ...
  final class MergedSourceLogic(_shape: Shape, producerCount: AtomicLong) extends GraphStageLogic(_shape) with OutHandler {
    ...
    private def onEvent(ev: Event): Boolean = ev match {
      ...
      case Deregister(id) ⇒
        demands.remove(id)
        true
      case CompletedStage(id) =>
        demands(id).completedStage = true
        if (completeWhenAllUpstreamsComplete && demands.valuesIterator.forall(_.completedStage)) {
          completeStage()
        }
        true
    }
    ...
    val sink = new GraphStage[SinkShape[T]] {
      ...
      override def createLogic(inheritedAttributes: Attributes): GraphStageLogic = new GraphStageLogic(shape) with InHandler {
        ...
        override def onUpstreamFinish(): Unit = {
          logic.enqueue(CompletedStage(id))
